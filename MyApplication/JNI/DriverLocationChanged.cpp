...
//    url +="/locationChanged";
    ...
    __android_log_print(ANDROID_LOG_ERROR, jniLogTag, "JNI-DriverLocationChanged: uri = [%s]\n", url);
    Curl curl(url, true);
    ...
    if (result.length() != 0) {
        if (result == "ok") {
            ...
            else if ((Data::getInstance()->getStatus() == OBO_STATUS_DRIVER_READY) && orderid != "NONE") {
                //已经有乘客下单 需要提示前端司机是否接单
                Data::getInstance()->setStatus(OBO_STATUS_DRIVER_READY);
            }
            else if ((Data::getInstance()->getStatus() == OBO_STATUS_DRIVER_DRIVE) && orderid == "NONE") {
                //说明订单已经结束，完成订单
