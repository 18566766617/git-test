...
JNIEXPORT jboolean
JNICALL Java_com_example_myapplication_OBOJNI_PassengerLocationChanged
        (JNIEnv *env, jobject obj, jstring jlongitude, jstring jlatitude, jstring jaddress,
...
//    url +="/locationChanged";
    ...
    char url[50] = {0};
    sprintf(url, "%s:%s/locationChanged", OBO_SERVER_IP, OBO_SERVER_PORT);
    __android_log_print(ANDROID_LOG_ERROR, jniLogTag, "JNI-locationChanged: uri = [%s]", url);
...
/*
    ...
    *
     ...
     //成功
     {
         ...
         order_status: "ACTIVE",则代表上车了

         //司机当前坐标，
         //供waiting和traveling的zhuangtai状态乘客使用，乘客端会返回该两个字段
